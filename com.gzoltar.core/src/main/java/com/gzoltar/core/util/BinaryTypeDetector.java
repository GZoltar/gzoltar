 ...
 */
public class BinaryTypeDetector {
  ...
  /**
   * Creates a new detector based on the given input. To process the complete original input
   * afterwards use the stream returned by {@link #getInputStream()}.
   * 
   * @param in input to read the header from
   * @throws IOException if the stream can't be read
   */
  public BinaryTypeDetector(final InputStream in) throws IOException {
    if (in.markSupported()) {
      this.in = in;
    } else {
  ...
  /**
   * Returns an input stream instance to read the complete content (including the header) of the
   * underlying stream.
   * 
