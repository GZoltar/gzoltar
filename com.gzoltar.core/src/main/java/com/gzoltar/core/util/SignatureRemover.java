...
public class SignatureRemover {
  ...
  private static final Pattern SIGNATURE_FILES = Pattern.compile("META-INF/[^/]*\\.SF|" //
      + "META-INF/[^/]*\\.DSA|" //
      + "META-INF/[^/]*\\.RSA|" //
      + "META-INF/SIG-[^/]*");
  ...
  /**
   * Defines whether this remover should be active. If it is not active it will not remove any
   * entries.
   * 
   * @param active <code>true</code> if it should remove signature related entries.
   */
   ...
   */
  public boolean filterEntry(final String name, final InputStream in, final OutputStream out)
      throws IOException {
